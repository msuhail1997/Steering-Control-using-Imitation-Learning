...
class ValidCarDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        center, left, right, speed = self.X[index]
        sa, throttle = self.y[index]
        image, sa = load_image(self.direc, center), float(sa)
        image = preprocess(image)

        if self.transform is not None:
            image = self.transform(image)
        speed = np.fromstring(str(speed[1:-2]), dtype=float, sep=',')
        throttle = float(throttle)
        return image, sa, speed, throttle#
...
class TrainCarDataset4Images(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return (image_center, sa_center, speed, throttle), (image_left, sa_left, speed, throttle), (image_right, sa_right, speed, throttle), (image_center_flip, sa_center_flip, speed, throttle)
