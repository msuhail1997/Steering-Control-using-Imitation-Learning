...
def generalize_image(image, sa):
    if np.random.rand() < 0.6:
        ...
        # image, sa = random_flip(image, sa)
        # image, sa = translate_image(image, sa)
...
class ValidCarDataset(data.Dataset):
    ...
    def __init__(self, X, y, direc, transform=None):
        ...
        self.transforms1 = transforms.Compose([transforms.ToTensor(),transforms.Lambda(lambda x: x/127.5 - 1)])
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            image = self.transforms1(transforms.functional.resize(image,size=(224,224)))
        return (image, sa)
...
class TrainCarDataset4Images(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            image_left = self.transforms1(transforms.functional.resize(image_left,size=(224,224)))
            image_center = self.transform(image_center)
            image_center = self.transforms1(transforms.functional.resize(image_center,size=(224,224)))
            image_right = self.transform(image_right)
            image_right = self.transforms1(transforms.functional.resize(image_right,size=(224,224)))
            image_center_flip = self.transform(image_center_flip)
            image_center_flip = self.transforms1(transforms.functional.resize(image_center_flip,size=(224,224)))
